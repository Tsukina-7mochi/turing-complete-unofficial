...
const createMenu = function(menu: Menu, titleLevel: number): SimpleHTMLElement[] {
  const htmlElements: SimpleHTMLElement[] = [];

  if(menu.title) {
    const title = new SimpleHTMLElement('h' + Math.max(1, Math.min(6, titleLevel)));
  ...
  menu.contents.forEach(content => {
    if(Object.prototype.hasOwnProperty.call(content, 'contents')) {
      ...
      item.appendChild(`<a href="${content}.html" class="page-link">${pageInfo.pages[content].title}</a>`);
      ...
      const item = new SimpleHTMLElement('li');
      item.appendChild(`<a href="${(<Link>content).href}" class="page-link">${(<Link>content).text}</a>`);
...
const generateMenuHTML = function(): string {
  return createMenu(pageInfo.menu, 1).map(el => el.toString()).join(' ');
}
