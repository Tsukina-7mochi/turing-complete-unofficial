  ...
  if (page.category === 'level' && page.dependencies) {
    ...
    const stringToInsert = [
      ...
      page.dependencies
        .map(
          (levelName) =>
            `<li><a href="${toLinkName(levelName)}.html" class="page-link">${
              pageInfo.pages[levelName].title
            }</a></li>`
        )
        .join(''),
    ...
    if (insertPos) {
      content =
        content.slice(0, insertPos) + stringToInsert + content.slice(insertPos);
    } else {
...
const loadMarkdown = function (pageName: string): string {
  ...
  const onCodeBlock = (
    lang: string,
    body: markdown.UTF8Bytes
  ): string | markdown.UTF8Bytes => {
    if (lang === 'truth_table') {
