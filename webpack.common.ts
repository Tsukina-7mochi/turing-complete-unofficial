...
const loadPages4HTMLWebpackPlugin = function(): HTMLWebpackPlugin[] {
  const pageNameFileExists = Object.keys(pageInfo.pages).filter((pageName) => {
    return fs.existsSync(path.join(docsPath, 'pages', pageInfo.pages[pageName].filename));
  });

  return pageNameFileExists.map((pageName) => new HTMLWebpackPlugin({
    filename: path.join(destPath, toLinkName(pageName) + '.html'),
    template: path.join(docsPath, 'template.hbs'),
    inject: false,
    templateParameters: {
      title: (pageName === 'index' ? 'Turing Complete Unofficial' : pageName + ' | Turing Complete Unofficial'),
      menu: menu,
