...
const pageInfo = json5.parse(fs.readFileSync(path.join(__dirname, '../docs/page_info.json5')));
...
for(const pageName in pageInfo.pages) {
  ...
  if(typeof page.title !== 'string') {
    console.error(`page ${pageName}: "title" must be string, not ${typeof page.title}`);
  }
  ...
  if(typeof page.filename !== 'string') {
    console.error(`page ${pageName}: "filename" must be string, not ${typeof page.filename}`);
  }
  ...
  if(!('category' in page)) {
    ...
    if(typeof page.category !== 'string') {
      ...
      if(page.category === 'level') {
        if(!('dependencies' in page)) {
          console.error(`page ${pageName} does not have "dependencies" field despite its category is "level".`);
        } else if(!Array.isArray(page.dependencies)) {
          console.error(`page ${pageName}: dependencies must be array, not ${typeof page.dependencies}`);
        } else {
          page.dependencies.forEach((dependency) => {
            if(typeof dependency !== 'string') {
              console.error(`page ${pageName}: item of dependencies must be string, but got ${dependency}(${typeof dependency})`);
            } else if(!(dependency in pageInfo.pages)) {
              console.error(`page ${pageName}: dependency page ${dependency} does not exist.`);
    ...
    for(const key in page) {
      if(!['title', 'filename', 'category', 'dependencies'].includes(key)) {
        ...
        console.error(`page ${pageName} has key "dependencies" despite its category is "${page.category}"`);
      }
    }
  }
}
...
if('alias' in pageInfo) {
  ...
  for(const key in pageInfo.alias) {
    if(typeof(pageInfo.alias[key]) !== 'string') {
      console.error(`value of pageInfo.alias.${key} must be string, not ${typeof(pageInfo.alias[key])}`);
    } else if(!(pageInfo.alias[key] in pageInfo.pages)) {
      console.error(`${pageInfo.alias[key]} (alias of ${key}) is not known page name.`);
    }
  }
}
...
const checkMenu = function(root, trace) {
  ...
  if(!('contents' in root)) {
    ...
    console.error(`${trace}: menu.contents must be array, not ${typeof(root.contents)}`);
  } else {
    root.contents.forEach((content, index) => {
      if(typeof(content) === 'string') {
        if(!(content in pageInfo.pages)) {
          console.error(`${trace}.contents[${index}]: ${content} is unknown page.`);
        }
      } else if(typeof(content) === 'object') {
        if('text' in content && 'href' in content) {
          if(typeof(content.text) !== 'string') {
            console.error(`${trace}.contents[${index}].text must be string, not ${content.text}`);
          } else if(typeof(content.href) !== 'string') {
            console.error(`${trace}.contents[${index}].text must be string, not ${content.href}`);
          }
        } else {
